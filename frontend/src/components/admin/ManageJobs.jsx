  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
          ...
          {(jobs || []).map((job) => (
            <div key={job._id} className="p-4 space-y-4">
              <div className="flex justify-between items-start">
                <div onClick={() => onViewDetails(job._id)} className="flex-1 cursor-pointer">
                  <h3 className="font-bold text-gray-900">{job.title}</h3>
                  <p className="text-xs text-gray-500 mt-0.5">Posted on {new Date(job.createdAt).toLocaleDateString()}</p>
                </div>
                <button 
                  onClick={() => toggleStatus(job._id, job.isActive)}
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => onViewDetails(job._id)}
                  className="flex-1 bg-blue-50 text-blue-600 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-blue-100 transition-colors"
                >
                ...
                <button 
                  onClick={() => onEdit(job)}
                  className="flex-1 bg-amber-50 text-amber-600 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-amber-100 transition-colors"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                ...
                <button 
                  onClick={() => deleteJob(job._id)}
                  className="flex-1 bg-red-50 text-red-600 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-red-100 transition-colors"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
        ...
        {(jobs || []).length === 0 && (
          <div className="px-6 py-10 text-center text-gray-400">No jobs posted yet.</div>
        )}
      </div>
  </div>
  );
